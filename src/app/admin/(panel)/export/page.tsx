...
export default async function ExportPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {rows.length === 0 ? (
        <p className="text-muted-foreground surface-card rounded-xl px-4 py-10 text-center">
        ...
        <div className="surface-card overflow-x-auto rounded-xl">
          <table className="w-full min-w-[44rem] text-sm">
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.variantId} className="border-border border-b last:border-0">
                  <td className="text-foreground px-4 py-2.5 font-medium">{row.productName}</td>
                  <td className="px-4 py-2.5">{row.color || '—'}</td>
                  <td className="px-4 py-2.5">{row.size || row.variantLabel || "—"}</td>
                  <td className="text-foreground px-4 py-2.5 text-right text-base font-semibold tabular-nums">{row.quantity}</td>
            ...
            <tfoot className="border-border text-foreground border-t font-semibold">
              <tr>
